locals {
  external_dns_name_prefix = "${module.itse-apps-prod-1.cluster_id}-external-dns"
...
resource "random_string" "string" {
  ...
  special = false
  keepers = {
    cluster_id = module.itse-apps-prod-1.cluster_id
  }
}
...
resource "aws_iam_policy" "external_dns" {
  ...
  path        = "/"
  description = "IAM Policy for external-dns on ${module.itse-apps-prod-1.cluster_id}"
