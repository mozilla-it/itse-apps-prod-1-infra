resource "aws_lambda_function" "tldr" {
  description   = "Post the weekly TL;DR email into Discourse ${var.environment}"
  ...
  environment {
    variables = {
      ...
      DISCOURSE_TLDR_URL          = "https://${var.discourse_mozilla}"
    }
  }
}
...
resource "aws_iam_role" "lambda_tldr" {
  name = "discourse-${var.environment}-lambda-tldr"
...
resource "aws_cloudwatch_log_group" "lambda_tldr" {
  name              = "/aws/lambda/discourse-${var.environment}-tldr"
  retention_in_days = 7
}

resource "aws_cloudwatch_log_subscription_filter" "lambda_tldr_papertrail" {
  name            = "discourse-${var.environment}-tldr-logs-to-papertrail"
...
resource "aws_s3_bucket" "tldr_email" {
  bucket = "discourse-${var.environment}-tldr-email"
  acl    = "private"
}

resource "aws_s3_bucket" "tldr_code" {
  bucket = "discourse-${var.environment}-tldr-code"
  acl    = "private"
}
...
resource "aws_ssm_parameter" "tldr_api_key" {
  name  = "/discourse/${var.environment}/tldr-api-key"
  type  = "String"
  value = "non-real-key"
