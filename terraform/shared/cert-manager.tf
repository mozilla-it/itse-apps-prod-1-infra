locals {
  ...
  cert_manager_name_prefix  = "${module.itse-apps-prod-1.cluster_id}-cert-manager"
...
resource "aws_iam_policy" "cert_manager" {
  ...
  path        = "/"
  description = "IAM Policy for cert-manager on ${module.itse-apps-prod-1.cluster_id}"
...
module "cert_manager_role" {
  ...
  provider_url                  = replace(module.itse-apps-prod-1.cluster_oidc_issuer_url, "https://", "")
...
EOF
    ...
    environment = {
      kube_config = base64encode(module.itse-apps-prod-1.kubeconfig)
    }
  }

  triggers = {
    endpoint = module.itse-apps-prod-1.cluster_endpoint
